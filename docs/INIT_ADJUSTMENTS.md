...
- [x] 评估是否把环境变量前缀 `CURATOR_` 迁移到 `INSIGHTLOOM_`(建议做兼容映射,避免中断)—— 已完成:`server/config.py` 统一读取,新前缀优先、旧前缀兼容;compose 嵌套默认值双透传;冒烟测试含兼容自检
- [ ] 在部署平台完成正式域名绑定并验证 OG 抓取
- [ ] 增加基础访问统计 —— 已定为 51.la(国内节点友好),等注册拿统计 ID
...
## 域名进度(2026-09-17 暂停,等身份证)

- insightloomapp.com 已确认可注册:**¥83/首年**,已加入购物车(腾讯云购物车会保留)
- 购买被注册局实名模板拦住:需上传身份证照片(暂时没有身份证在手)
- 账号信息联动已完成:勾「使用账号信息填写」可自动带出姓名/证件号/手机
- 备案同样需要身份证 → 拿到证件后 30 分钟可跑完:模板→购买→备案提交
- 期间线上访问:EdgeOne 默认域名(控制台「部署→预览」随时生成 3 小时链接)
